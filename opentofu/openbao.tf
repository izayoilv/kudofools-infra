...
resource "vault_kubernetes_auth_backend_role" "eso" {
  ...
  token_policies                   = [vault_policy.woodpecker.name, vault_policy.forgejo.name, vault_policy.kudofools_infra.name, vault_policy.matrix_conduit.name, vault_policy.element_web.name, vault_policy.lldap.name, vault_policy.zot.name, vault_policy.rathole.name, vault_policy.mgb_temple_static.name, vault_policy.mgb_temple_cms.name, vault_policy.vaultwarden.name]
  token_ttl                        = 3600
}
...
resource "vault_policy" "mgb_temple_cms" {
  name   = "mgb-temple-cms"
  policy = <<EOT
path "kv/data/mgb-temple-cms/*" {
  capabilities = ["read"]
}
EOT
}
